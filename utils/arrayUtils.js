...
const log = createLogger('ARRAY_UTILS');
 ...
 */
function removeDuplicatesById(array, keyField = 'id', options = {}) {
    const {
    ...
    array.forEach(item => {
        ...
        if (!uniqueMap.has(key)) {
            ...
            if (warnOnDuplicate) {
                ...
                log.warn(`[${context}] Dublikat topildi: ${keyField}=${key}, Name=${itemName}`);
            }
        }
    });
    ...
    if (duplicateIds.length > 0 && warnOnDuplicate) {
        ...
        log.warn(`[${context}] Dublikatlar: Original=${array.length}, Unique=${result.length}, Dublikatlar=${duplicateIds.length}, Unique IDs=${uniqueIds.join(', ')}`);
    }
 ...
 * @param {Array} array - Dublikatlar bo'lishi mumkin bo'lgan array
 * @param {string[]} keyFields - Unique key field nomlari (masalan: ['brand_id', 'branch_id'])
 * @param {Object} options - Qo'shimcha sozlamalar
 * @returns {Array} - Unique array
 */
function removeDuplicatesByKeys(array, keyFields = [], options = {}) {
    const {
        warnOnDuplicate = false,
        context = '',
        sortFn = null
    ...
    array.forEach(item => {
        ...
        if (!uniqueMap.has(compositeKey)) {
            ...
            if (warnOnDuplicate) {
                ...
                log.warn(`[${context}] Dublikat topildi: ${keyValues}`);
            }
        }
    });
    ...
    if (duplicateKeys.length > 0 && warnOnDuplicate) {
        log.warn(`[${context}] Dublikatlar: Original=${array.length}, Unique=${result.length}, Dublikatlar=${duplicateKeys.length}`);
    }
 ...
 */
function filterAndUnique(array, filterFn, keyField = 'id') {
    if (!Array.isArray(array) || array.length === 0) {
        return array;
    }
