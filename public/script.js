...
const renderKpiCards = (stats) => {
    let kpiCardsHtml = '';
    // Umumiy KPI - To'q ko'k rang (kechikkan va tahrirlanganlar umumiy KPI dan ayiriladi)
    const latePenaltyValue = stats.latePenalty || 0.5;
    const editPenaltyValue = stats.editPenalty || 0.3;
    const lateDeduction = Math.round((stats.latePercent || 0) * latePenaltyValue);
    const editDeduction = Math.round((stats.editedPercent || 0) * editPenaltyValue);
    const baseKpi = stats.ontimePercent || 0;
    ...
    kpiCardsHtml += `
        <div class="kpi-card kpi-main-card" style="background: linear-gradient(135deg, #2563eb, #1e40af); box-shadow: 0 6px 16px rgba(37, 99, 235, 0.35);">
            ...
            <div class="kpi-card-info" style="flex: 1;">
                ...
                <div style="font-size: 11px; opacity: 0.85; margin-top: 4px; line-height: 1.4;">
                    <div style="display: flex; justify-content: space-between; margin-bottom: 2px;">
                        <span>O'z vaqtida:</span>
                        <span style="font-weight: 600;">${baseKpi}%</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; margin-bottom: 2px; color: rgba(255, 200, 200, 0.9);">
                        <span>Kechikkan (-${latePenaltyValue * 100}%):</span>
                        <span style="font-weight: 600;">-${lateDeduction}%</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; color: rgba(255, 230, 200, 0.9);">
                        <span>Tahrirlangan (-${editPenaltyValue * 100}%):</span>
                        <span style="font-weight: 600;">-${editDeduction}%</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="kpi-card" style="background: linear-gradient(135deg, #16a34a, #15803d); box-shadow: 0 6px 16px rgba(22, 163, 74, 0.35);">
            ...
            <div class="kpi-card-info">
                ...
                <div class="kpi-card-value-wrapper">
                    ...
                    <span class="kpi-percent">${stats.ontimePercent || 0}%</span>
                </div>
            </div>
        </div>
    `;
   ...
   // --- Asosiy Funksiyalar ---
    ...
    function applyRolePermissions() {
        ...
        // MUAMMO TUZATILDI: Agar reports:edit_all bo'lsa, reports:edit_own va reports:edit_assigned ham qo'shiladi
        if (userPermissions.includes('reports:edit_all')) {
            if (!virtualPermissions.includes('reports:edit_own')) {
                virtualPermissions.push('reports:edit_own');
            }
            if (!virtualPermissions.includes('reports:edit_assigned')) {
                virtualPermissions.push('reports:edit_assigned');
            }
        }
        
        // Agar reports:edit_assigned bo'lsa, reports:edit_own ham qo'shiladi
        if (userPermissions.includes('reports:edit_assigned')) {
            if (!virtualPermissions.includes('reports:edit_own')) {
                virtualPermissions.push('reports:edit_own');
            }
        }
        ...
        document.querySelectorAll('[data-permission]').forEach(el => {
            ...
            if (!hasPermission) {
                ...
                el.style.display = ''; // Display'ni qayta tiklash
            }
        });
    }
    ...
    function renderSavedReports() {
        ...
        const htmlContent = reportIds.map(id => {
            ...
            let canDelete = false;
            if (hasDeletePermission && state.currentUser) {
                // Superadmin barcha hisobotlarni o'chira oladi
                if (isSuperAdmin) {
                    canDelete = true;
                    console.log(`[DEBUG] Delete permission check (script.js): Report ${id} - Superadmin, canDelete = true`);
                } else {
                    ...
                    console.log(`[DEBUG] Delete permission check (script.js): Report ${id} - canDeleteAll: ${canDeleteAll}, canDeleteAssigned: ${canDeleteAssigned}, canDeleteOwn: ${canDeleteOwn}, hasOldDelete: ${hasOldDelete}, canDelete: ${canDelete}`);
                }
            } else {
                console.log(`[DEBUG] Delete permission check (script.js): Report ${id} - hasDeletePermission: ${hasDeletePermission}, currentUser: ${!!state.currentUser}`);
            }
            ...
            console.log(`[DEBUG] Delete button HTML (script.js): Report ${id} - canDelete: ${canDelete}, deleteBtn length: ${deleteBtn.length}`);
    ...
    function updateUIForReportState() {
        ...
        // Debug uchun
        console.log('[DEBUG] updateUIForReportState (script.js):', {
            isNew,
            ...
            DOM_editBtn_exists: !!DOM.editBtn
        }); // <-- BU QAVS QO'SHILISHI KERAK
        ...
        console.log('[DEBUG] Edit permissions check (script.js):', {
            hasEditAll,
            hasEditAssigned,
            hasEditOwn,
            canEdit,
            ...
            isEditMode: state.isEditMode,
            hasReport: !!report,
            shouldShowEditBtn: !isNew && !state.isEditMode && canEdit,
            DOM_editBtn_exists: !!DOM.editBtn,
            editBtn_currentClasses: DOM.editBtn ? DOM.editBtn.className : 'N/A'
        });
        ...
        if (DOM.editBtn) {
            const shouldHide = isNew || state.isEditMode || !canEdit;
            console.log('[DEBUG] Edit button visibility decision:', {
                shouldHide,
                reason: {
                    isNew,
                    isEditMode: state.isEditMode,
                ...
                buttonElement: DOM.editBtn,
                beforeClasses: DOM.editBtn.className
            });
            DOM.editBtn.classList.toggle('hidden', shouldHide);
            console.log('[DEBUG] Edit button after toggle:', {
                afterClasses: DOM.editBtn.className,
                isHidden: DOM.editBtn.classList.contains('hidden')
            });
        } else {
            console.error('[DEBUG] ❌ DOM.editBtn topilmadi! Element mavjud emas.');
        }
    ...
    // Helper funksiya: O'chirish ruxsatini tekshirish
    function canDeleteReport(report) {
        if (!state.currentUser || !report) return false;
        
        const isSuperAdmin = state.currentUser.role === 'superadmin' || state.currentUser.role === 'super_admin';
        if (isSuperAdmin) return true;
        
        const deletePermissions = ['reports:delete_all', 'reports:delete_assigned', 'reports:delete_own', 'reports:delete'];
        const hasDeletePermission = deletePermissions.some(p => state.currentUser.permissions?.includes(p));
        
        if (!hasDeletePermission) return false;
        
        const canDeleteAll = state.currentUser.permissions?.includes('reports:delete_all');
        const reportLocation = report.location || report.old_location;
        const canDeleteAssigned = state.currentUser.permissions?.includes('reports:delete_assigned') && 
            state.currentUser.locations && Array.isArray(state.currentUser.locations) && 
            reportLocation && state.currentUser.locations.includes(reportLocation);
        const canDeleteOwn = state.currentUser.permissions?.includes('reports:delete_own') && 
            report.created_by && String(report.created_by) === String(state.currentUser.id);
        const hasOldDelete = state.currentUser.permissions?.includes('reports:delete');
        
        return canDeleteAll || canDeleteAssigned || canDeleteOwn || hasOldDelete;
    }

    // Helper funksiya: O'chirish tugmasi HTML yaratish
    function getDeleteButtonHTML(reportId, report) {
        if (!canDeleteReport(report)) return '';
        
        return `
            <button class="kpi-delete-btn" data-id="${reportId}" title="O'chirish" onclick="event.stopPropagation(); deleteReport('${reportId}')">
                <i data-feather="trash-2"></i>
            </button>`;
    }
...
// --- Modern Confirm Dialog ---
    ...
    function setupEventListeners() {
        ...
        // Mobile menu toggle - olib tashlangan
        ...
        // Mobile menu toggle olib tashlangan, shuning uchun event listener ham olib tashlandi
    ...
    async function loadKPIStats() {
        try {
            // KPI sozlamalarini yuklash
            try {
                const settingsRes = await fetch('/api/settings');
                if (settingsRes && settingsRes.ok) {
                    const settingsData = await settingsRes.json();

                    // /api/settings hozir obyekt qaytaradi (settings.js da), lekin kelajakda array bo'lishi ham mumkin.
                    let rawKpiSettings = null;

                    if (Array.isArray(settingsData)) {
                        const kpiRow = settingsData.find(s => s.key === 'kpi_settings');
                        rawKpiSettings = kpiRow ? kpiRow.value : null;
                    } else if (settingsData && Object.prototype.hasOwnProperty.call(settingsData, 'kpi_settings')) {
                        rawKpiSettings = settingsData.kpi_settings;
                    }

                    if (rawKpiSettings) {
                        // Agar string bo'lsa JSON.parse qilamiz, aks holda obyektni to'g'ridan-to'g'ri ishlatamiz
                        window.kpiSettings = typeof rawKpiSettings === 'string'
                            ? JSON.parse(rawKpiSettings)
                            : rawKpiSettings;
                    } else {
                        window.kpiSettings = { latePenalty: 0.5, editPenalty: 0.3 };
                    }
                } else {
                    window.kpiSettings = { latePenalty: 0.5, editPenalty: 0.3 };
                }
            } catch (error) {
                console.warn('KPI sozlamalarini yuklashda xatolik, default qiymatlar ishlatiladi:', error);
                window.kpiSettings = { latePenalty: 0.5, editPenalty: 0.3 };
            }
            ...
            // Foizlarni hisoblash
            const onTimePercentage = total > 0 ? (ontime.length / total) * 100 : 0;
            const latePercentage = total > 0 ? (late.length / total) * 100 : 0;
            const editedPercentage = total > 0 ? (edited.length / total) * 100 : 0;
            
            // KPI sozlamalarini olish (default: latePenalty = 0.5, editPenalty = 0.3)
            const kpiSettings = window.kpiSettings || { latePenalty: 0.5, editPenalty: 0.3 };
            const latePenalty = kpiSettings.latePenalty || 0.5;
            const editPenalty = kpiSettings.editPenalty || 0.3;
            
            // Umumiy KPI = o'z vaqtida foizi - (kechikkan foizi * penalty) - (tahrirlangan foizi * penalty)
            // Minimum 0% bo'lishi kerak
            const totalKpiPercent = Math.max(0, Math.round(
                onTimePercentage - (latePercentage * latePenalty) - (editedPercentage * editPenalty)
            ));
            ...
            const stats = {
                ...
                ontimePercent: Math.round(onTimePercentage),
                late: late.length,
                latePercent: Math.round(latePercentage),
                edited: edited.length,
                editedPercent: Math.round(editedPercentage),
    ...
    function openTotalKPIModal() {
        ...
        if (!window.kpiData || !window.kpiData.reports || !window.kpiData.reports.length) {
            ...
            list.innerHTML = filteredReports.map(report => {
                ...
                const deleteBtn = getDeleteButtonHTML(report.id, report);
